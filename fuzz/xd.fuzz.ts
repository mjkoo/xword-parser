...
function validateXdParsed(parsed: XdPuzzle): string | null {
  if (!Array.isArray(parsed.grid)) return "grid is not an array";
  if (parsed.grid.length === 0) return "grid is empty";

  const width = parsed.grid[0]?.length ?? 0;
  for (let row = 0; row < parsed.grid.length; row++) {
    const gridRow = parsed.grid[row]!;
    if (!Array.isArray(gridRow)) return `row ${row} is not an array`;
    if (gridRow.length !== width)
      return `row ${row} length (${gridRow.length}) !== width (${width})`;
    for (const cell of gridRow) {
      if (typeof cell !== "string") return `cell in row ${row} is not a string`;
      if (cell.length < 1) return `cell in row ${row} is empty`;
    }
  }

  if (!Array.isArray(parsed.across)) return "across is not an array";
  for (const clue of parsed.across) {
    if (typeof clue.number !== "string")
      return "across clue number is not a string";
    if (typeof clue.clue !== "string")
      return "across clue text is not a string";
    if (typeof clue.answer !== "string")
      return "across clue answer is not a string";
  }

  if (!Array.isArray(parsed.down)) return "down is not an array";
  for (const clue of parsed.down) {
    if (typeof clue.number !== "string")
      return "down clue number is not a string";
    if (typeof clue.clue !== "string") return "down clue text is not a string";
    if (typeof clue.answer !== "string")
      return "down clue answer is not a string";
  }
  ...
  if (parsed.metadata !== undefined) {
    ...
    for (const [key, value] of Object.entries(parsed.metadata)) {
      if (typeof key !== "string") return "metadata key is not a string";
      if (value !== undefined && typeof value !== "string")
        return "metadata value is not a string";
    }
  }
...
fuzz("validates error handling and data integrity", (data: Buffer) => {
  ...
  expect(validateXdParsed(parsed)).toBeNull();
