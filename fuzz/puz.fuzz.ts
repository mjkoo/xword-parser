...
function validatePuzParsed(parsed: PuzPuzzle): string | null {
  if (typeof parsed.width !== "number") return "width is not a number";
  if (typeof parsed.height !== "number") return "height is not a number";
  if (parsed.width <= 0) return "width is not positive";
  if (parsed.height <= 0) return "height is not positive";
  if (parsed.width > 255) return "width exceeds 255";
  if (parsed.height > 255) return "height exceeds 255";
  ...
  for (let row = 0; row < parsed.height; row++) {
    ...
    for (let col = 0; col < parsed.width; col++) {
      ...
      if (
        cell.playerState !== undefined &&
        typeof cell.playerState !== "string"
      )
        return `cell[${row}][${col}].playerState is not string`;
    }
  }
  ...
  if (
    ...
    return "metadata.title is not a string";
  if (
    parsed.metadata.author !== undefined &&
    typeof parsed.metadata.author !== "string"
  )
    return "metadata.author is not a string";
  if (
    parsed.metadata.copyright !== undefined &&
    typeof parsed.metadata.copyright !== "string"
  )
    return "metadata.copyright is not a string";
  if (
    parsed.metadata.notes !== undefined &&
    typeof parsed.metadata.notes !== "string"
  )
    return "metadata.notes is not a string";

  if (
    parsed.isScrambled !== undefined &&
    typeof parsed.isScrambled !== "boolean"
  )
    return "isScrambled is not boolean";

  if (parsed.rebusTable !== undefined) {
    if (!(parsed.rebusTable instanceof Map)) return "rebusTable is not a Map";
...
fuzz("validates error handling and data integrity", (data: Buffer) => {
  ...
  expect(validatePuzParsed(parsed)).toBeNull();
