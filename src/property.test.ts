...
describe("Property-based tests", () => {
  describe("iPUZ parser properties", () => {
    ...
    test.prop([
      ...
      "should handle invalid grid dimensions gracefully with proper error types",
      (puzzleData) => {
        const ipuz = {
          ...puzzleData,
          puzzle: [],
        ...
        try {
          ...
          const unified = convertIpuzToUnified(parsed);
          if (
            puzzleData.dimensions.width <= 0 ||
            puzzleData.dimensions.height <= 0
          ) {
          ...
          const isExpectedError =
            error instanceof ParseError || error instanceof IpuzParseError;
  ...
  describe("JPZ parser properties", () => {
    test.prop([
      ...
      "should parse valid JPZ XML puzzles",
...
<crossword-compiler-applet>
  <rectangular-puzzle>
    ...
    test.prop([
      ...
      "should handle malformed JPZ XML without low-level errors",
      (xmlContent) => {
        try {
          parseJpz(xmlContent);
        } catch (e) {
  ...
  describe("PUZ parser properties", () => {
    test.prop([
      ...
      try {
        ...
        const error = e as Error;
        expect(
          error instanceof PuzParseError || error instanceof InvalidFileError,
        ).toBe(true);
      }
    });
  ...
  describe("XD parser properties", () => {
    test.prop([
      ...
      expect(parsed).toBeDefined();
      expect(parsed.metadata.title?.trim() || undefined).toBe(
        title.trim() || undefined,
      );
      expect(parsed.metadata.author?.trim() || undefined).toBe(
        author.trim() || undefined,
      );

      const unified = convertXdToUnified(parsed);
      expect(unified.title?.trim() || undefined).toBe(
        title.trim() || undefined,
      );
      expect(unified.author?.trim() || undefined).toBe(
        author.trim() || undefined,
      );
    });
    ...
    test.prop([
      ...
      ({ title, author, width, height }) => {
        ...
        try {
          ...
          const error = e as Error;
          const isExpectedError =
            error instanceof ParseError || error instanceof XdParseError;
  ...
  describe("Robustness against low-level errors", () => {
    test.prop([
      ...
      "parsers must handle malformed grids without low-level errors",
      (puzzleData) => {
        const ipuz = {
          ...puzzleData,
          clues: { Across: [], Down: [] },
    ...
    test.prop([
      ...
      "parsers must handle out-of-bounds access safely",
      ({ grid, accessY, accessX }) => {
        const ipuz = {
          version: "http://ipuz.org/v2",
          kind: ["http://ipuz.org/crossword#1"],
  ...
  describe("Unicode and special character handling", () => {
    test.prop([
      ...
      "parsers should handle Unicode in metadata and clues",
      ({ title, author, clueText }) => {
        const ipuz = {
          version: "http://ipuz.org/v2",
          kind: ["http://ipuz.org/crossword#1"],
          title,
