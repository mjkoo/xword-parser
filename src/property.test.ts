...
describe('Property-based tests', () => {
  describe('iPUZ parser properties', () => {
    // Test with realistic grid sizes up to 50x50
    test.prop([
      fc.record({
      ...
      // Generate a simple valid puzzle grid
      const puzzle: unknown[][] = [];
      for (let y = 0; y < height; y++) {
        const row: unknown[] = [];
        for (let x = 0; x < width; x++) {
          // Simple pattern: alternate between empty and black
          row.push((x + y) % 3 === 0 ? '#' : 0);
        }
        puzzle.push(row);
      }
    ...
    // Test edge cases for grid dimensions
    test.prop([
      fc.record({
        version: fc.constant('http://ipuz.org/v2'),
        kind: fc.constant(['http://ipuz.org/crossword#1']),
        dimensions: fc.oneof(
          fc.record({ width: fc.integer({ min: -10, max: 0 }), height: fc.integer({ min: 1, max: 10 }) }),
          fc.record({ width: fc.integer({ min: 1, max: 10 }), height: fc.integer({ min: -10, max: 0 }) }),
          fc.record({ width: fc.integer({ min: 100, max: 1000 }), height: fc.integer({ min: 100, max: 1000 }) })
        )
      })
      ...
      // Should either parse successfully or throw our custom error types
      try {
        const parsed = parseIpuz(jsonString);
        const unified = convertIpuzToUnified(parsed);
        // If we get here with negative/zero dimensions, it should have created an empty grid
        if (puzzleData.dimensions.width <= 0 || puzzleData.dimensions.height <= 0) {
          expect(unified.grid.cells.length).toBeGreaterThanOrEqual(0);
        }
      } catch (e) {
        // Must be one of our error types, not a generic Error
  ...
  describe('JPZ parser properties', () => {
    // Property: JPZ parser should handle various XML structures
    test.prop([
      fc.record({
...
<crossword-compiler-applet>
  <rectangular-puzzle>
    ...
    <crossword>
      ...
      try {
        ...
        // Must be our error type
        const error = e as Error;
        expect(error instanceof JpzParseError).toBe(true);
      }
    });

    // Property: JPZ parser must handle malformed XML gracefully
    test.prop([
      fc.oneof(
        fc.constant('<crossword>'),  // Unclosed tag
        fc.constant('<?xml version="1.0"?><grid/>'),  // Missing puzzle structure
        fc.constant('<rectangular-puzzle></rectangular-puzzle>'),  // Empty puzzle
        fc.string({ maxLength: 100 }).map(s => `<puzzle>${s}</puzzle>`)  // Random content
      )
      ...
      try {
        ...
        const error = e as Error;
        // Should not throw low-level errors
  ...
  describe('PUZ parser properties', () => {
    // Property: PUZ parser should handle binary data correctly
    test.prop([
      fc.record({
        width: fc.integer({ min: 3, max: 30 }),
        height: fc.integer({ min: 3, max: 30 })
      ...
      // Create a minimal valid PUZ structure
      // PUZ format has complex binary structure, so we'll test with pre-generated valid headers
      const headerSize = 52;
      const gridSize = width * height;
      const stringSize = 100; // Approximate for title, author, copyright
      
      // Create a buffer with valid PUZ magic bytes
      const buffer = Buffer.alloc(headerSize + gridSize * 2 + stringSize);
      
      // Magic string "ACROSS&DOWN\0"
      buffer.write('ACROSS&DOWN\0', 0x02);
      
      // Width and height at specific offsets
      buffer.writeUInt8(width, 0x2C);
      buffer.writeUInt8(height, 0x2D);
      
      // Fill grid with valid characters
      for (let i = 0; i < gridSize; i++) {
        buffer.writeUInt8(i % 26 + 65, headerSize + i); // A-Z
        buffer.writeUInt8(i % 26 + 65, headerSize + gridSize + i); // Solution
      }
    ...
    // Property: PUZ parser must reject invalid binary data gracefully
    test.prop([
      fc.uint8Array({ minLength: 10, maxLength: 100 })
    ])('should handle random binary data without crashes', (bytes) => {
  ...
  describe('XD parser properties', () => {
    test.prop([
      ...
      expect(parsed).toBeDefined();
      // XD parser may trim whitespace from title/author
    ...
    // Test edge cases for XD dimensions
    test.prop([
      fc.record({
    ...
    ])('should handle invalid XD dimensions gracefully with proper error types', ({ title, author, width, height }) => {
      // For invalid dimensions, parsing should either succeed or throw our custom error types
      const xdContent = [
        `Title: ${title}`,
        `Author: ${author}`,
        `Size: ${width}x${height}`,
      ...
      try {
        const parsed = parseXd(xdContent);
        // If it parses with invalid dimensions, check it handles them
        expect(parsed).toBeDefined();
      } catch (e) {
  ...
  describe('Grid consistency properties', () => {
    ...
    // Property: Grid must always be rectangular (all rows same length)
    test.prop([
      fc.string({ minLength: 1, maxLength: 1000 })
    ])('all parsed grids must be rectangular', (content) => {
      // Test iPUZ parser
      try {
        ...
        if (grid.cells.length > 0) {
          ...
          if (firstRow) {
            ...
            // Dimensions should match actual grid
            expect(grid.height).toBe(grid.cells.length);
            expect(grid.width).toBe(expectedWidth);
          }
        }
      } catch (e) {
        // If parsing fails, ensure it's our error type
        const error = e as Error;
        const isExpectedError = 
          error instanceof XwordParseError ||
          error instanceof IpuzParseError;
    ...
    // Property: Width and height should always match actual grid dimensions
    test.prop([
      fc.integer({ min: 1, max: 50 }),
      fc.integer({ min: 1, max: 50 })
    ])('grid width/height must match actual cell array dimensions', (width, height) => {
      const ipuz = {
        version: 'http://ipuz.org/v2',
        kind: ['http://ipuz.org/crossword#1'],
        dimensions: { width, height },
    ...
    // Property: Every cell must have required properties
    test.prop([
      fc.integer({ min: 1, max: 20 }),
      fc.integer({ min: 1, max: 20 })
    ])('every cell must have isBlack property defined', (width, height) => {
      const ipuz = {
        version: 'http://ipuz.org/v2',
        kind: ['http://ipuz.org/crossword#1'],
        dimensions: { width, height },
  ...
  describe('Clue consistency properties', () => {
    // Property: All clue numbers must be positive integers
    test.prop([
      fc.array(
        fc.record({
          number: fc.oneof(
    ...
    // Property: Clue text should never be undefined
    test.prop([
      fc.array(
        fc.record({
  ...
  describe('Robustness against low-level errors', () => {
    // Property: Parsers must not throw on missing or malformed grids
    test.prop([
      fc.record({
        version: fc.constant('http://ipuz.org/v2'),
        kind: fc.constant(['http://ipuz.org/crossword#1']),
    ...
    // Property: Array access should never throw
    test.prop([
      fc.record({
        grid: fc.array(
  ...
  describe('Parse function detection properties', () => {
    ...
    ])('should attempt iPUZ parsing for JSON-like content with proper error types', ({ prefix, version, suffix }) => {
    ...
    ])('should attempt JPZ parsing for XML-like content with proper error types', ({ tagName, content }) => {
  ...
  describe('Solution data consistency', () => {
    // Property: Solution grid must match puzzle grid dimensions
    test.prop([
      fc.integer({ min: 3, max: 20 }),
      fc.integer({ min: 3, max: 20 })
    ])('solution grid must match puzzle grid dimensions', (width, height) => {
    ...
    // Property: Black cells should not have solutions
    test.prop([
      fc.integer({ min: 3, max: 20 }),
      fc.integer({ min: 3, max: 20 })
    ])('black cells should not have solutions', (width, height) => {
      const puzzle = Array.from({ length: height }, (_, y) => 
        Array.from({ length: width }, (_, x) => {
  ...
  describe('Unicode and special character handling', () => {
    // Property: Parsers should handle Unicode characters in text fields
    test.prop([
      fc.record({
        title: fc.oneof(
    ...
    // Property: Special characters in grid solutions
    test.prop([
      fc.array(
        fc.constantFrom('A', 'B', 'C', 'D', 'E', 'É', 'Ñ', '©', '™', '→', '♠', 'Z'),
        { minLength: 9, maxLength: 9 }
      )
    ])('should handle special characters in solutions', (solutions) => {
      const puzzle = [[0, 0, 0], [0, '#', 0], [0, 0, 0]];
      const solution = [
  ...
  describe('Cross-format consistency', () => {
    // Property: Converting to unified and back should preserve essential data
    test.prop([
      fc.record({
    ...
    ])('essential puzzle data should be preserved across conversions', ({ width, height, title, author }) => {
  ...
  describe('Error handling', () => {
    ...
    test.prop([
    ...
    ])('all parsers should only throw errors from our hierarchy for string inputs', (content) => {
      ...
      for (const parser of parsers) {
        try {
          ...
          if (!isExpectedError) {
            throw new Error(`${parser.name} threw unexpected error type: ${error.constructor.name} - ${error.message}`);
          }
        }
      }
    });
    ...
    ])('all parsers should only throw errors from our hierarchy for Buffer inputs', (bytes) => {
      const buffer = Buffer.from(bytes);
