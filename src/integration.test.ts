...
describe("End-to-End Workflows", () => {
  describe("Complete parsing workflow", () => {
      ...
      for (const format of formats) {
        ...
        for (const file of filesToTest) {
          ...
          try {
            ...
            console.log(
              `Skipping ${file}: ${e instanceof Error ? e.message : String(e)}`,
            );
          }
        }
      }
    ...
    test("should handle mixed format detection without hints", () => {
      const testFiles = [
        {
          path: join(__dirname, "..", "testdata", "ipuz", "example.ipuz"),
          format: "ipuz",
        },
        {
          path: join(__dirname, "..", "testdata", "puz", "nyt_locked.puz"),
          format: "puz",
        },
        {
          path: join(__dirname, "..", "testdata", "jpz", "FM.jpz"),
          format: "jpz",
        },
        {
          path: join(__dirname, "..", "testdata", "xd", "atc2003-09-03.xd"),
          format: "xd",
        },
      ];
    ...
    test("should produce consistent results with sync and async parsing", async () => {
      ...
      const asyncPuzzle = await parseLazy(content, {
        filename: "example.ipuz",
      });
  ...
  describe("Format-specific to unified conversion", () => {
    test("should maintain data integrity through format-specific parsing and conversion", () => {
      ...
      if (puzPuzzle.metadata.title)
        expect(puzUnified.title).toBe(puzPuzzle.metadata.title);
      if (puzPuzzle.metadata.author)
        expect(puzUnified.author).toBe(puzPuzzle.metadata.author);
      ...
      if (jpzPuzzle.metadata.title)
        expect(jpzUnified.title).toBe(jpzPuzzle.metadata.title);
      if (jpzPuzzle.metadata.creator)
        expect(jpzUnified.author).toBe(jpzPuzzle.metadata.creator);
      ...
      if (xdPuzzle.metadata.title)
        expect(xdUnified.title).toBe(xdPuzzle.metadata.title);
      if (xdPuzzle.metadata.author)
        expect(xdUnified.author).toBe(xdPuzzle.metadata.author);
    });
  });
  ...
  describe("Error handling workflow", () => {
    ...
    test("should provide helpful error messages for format mismatches", () => {
      const ipuzContent = readFileSync(
