...
describe('ipuz parser', () => {
  ...
  function assertCellTypes(puzzle: IpuzPuzzle, filename: string) {
    ...
    puzzle.puzzle.forEach((row, y) => {
      row.forEach((cell, x) => {
        ...
        expect(Object.values(CellType), `${filename}: invalid cell type at [${y}][${x}]`).toContain(
          cell.type,
        );
  ...
  function assertClueStructure(puzzle: IpuzPuzzle, filename: string) {
    Object.entries(puzzle.clues).forEach(([direction, clues]) => {
      expect(Array.isArray(clues), `${filename}: clues for ${direction} should be array`).toBe(
        true,
      );
      ...
      clues.forEach((clue, i) => {
        ...
        expect(typeof clue.text, `${filename}: ${direction}[${i}] text should be string`).toBe(
          'string',
        );
      });
    });
  ...
  describe('data integrity', () => {
    testFiles.forEach((filename) => {
      ...
      it(`${filename} should have valid cell types`, () => {
        ...
        puzzle.puzzle.forEach((row, y) => {
          row.forEach((cell, x) => {
            expect(Object.values(CellType), `Invalid cell type at [${y}][${x}]`).toContain(
              cell.type,
            );
          });
        });
